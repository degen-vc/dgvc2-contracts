...
describe("DGVC", function () {
  ...
  it("should be possible to transfer tokens, fees are not set", async function () {
    ...
    expect(await dgvcProxy.balanceOf(owner.address)).to.equal(
      totalSupply - amount
    );
  ...
  it("should be possible to transfer tokens, fees set to 5%, 2.5% - to burn and 2.5% fot, trade cycle updated", async function () {
    ...
    expect(await dgvcProxy.actualBurnCycle()).to.equal(
      (amount * partFee) / HUNDRED_PERCENT
    );
    expect(await dgvcProxy.totalBurn()).to.equal(
      (amount * partFee) / HUNDRED_PERCENT
    );
    ...
    expect(await dgvcProxy.balanceOf(user.address)).to.equal(
      amount - (amount * fee) / HUNDRED_PERCENT
    );
    expect(
      await dgvcProxy.balanceOf(feeReceiver.address),
      (amount * partFee) / HUNDRED_PERCENT
    );

    expect(await dgvcProxy.totalSupply()).to.equal(
      totalSupply - (amount * partFee) / HUNDRED_PERCENT
    );
  });
  ...
  it("should be possible to transfer tokens, trade cycle reached, fees auto set to 5.5%, 2.75% - to burn and 2.75% fot", async function () {
    ...
    expect(await dgvcProxy.balanceOf(feeReceiver.address)).to.equal(
      (amount * partFee) / HUNDRED_PERCENT
    );
  ...
  it("should be possible reach 15 trade cycles, fees auto set to 12%, 6% - to burn and 6% fot, check total supply after rebase", async function () {
    ...
    const totalSupplyExpected =
      supplyBeforeRebase.toBigInt() +
      rebaseAmount -
      (amount * fee) / HUNDRED_PERCENT;
    ...
    expect(
      BigInttoBN(
        BNtoBigInt(balanceOwner) +
          BNtoBigInt(balanceUser) +
          BNtoBigInt(balanceFeeReceiver)
      )
    ).to.equal(BigInttoBN(totalSupplyExpected - 1n));
  ...
  it("should be possible to make an admin burn for feeReceiver without trade cycles", async function () {
    ...
    await expect(
      dgvcProxy.connect(feeReceiver).burn(feeReceiverBalance)
    ).to.emit(dgvcProxy, "Transfer");

    expect(await dgvcProxy.balanceOf(feeReceiver.address)).to.equal(0);
    expect(await dgvcProxy.totalSupply()).to.equal(
      totalSupply - (amount * partFee) / HUNDRED_PERCENT - feeReceiverBalance
    );
    expect(await dgvcProxy.totalBurn()).to.equal(
      (amount * partFee) / HUNDRED_PERCENT + feeReceiverBalance
    );
  });
  ...
  it("should NOT be possible to make an admin burn for NOT feeReceiver without trade cycles.", async function () {
    ...
    await expect(
      dgvcProxy.connect(user).burn(feeReceiverBalance)
    ).to.revertedWith("Only feeReceiver");
  });
  ...
  it("should NOT be possible to make an admin burn for feeReceiver does not have amount tokens.", async function () {
    ...
    await expect(
      dgvcProxy.connect(feeReceiver).burn(feeReceiverBalance + 1n)
    ).to.revertedWith("Cannot burn more than on balance");
  });
  ...
  it("transfer to same user should not multiply user balance / total supply", async function () {
    ...
    expect(await dgvcProxy.balanceOf(owner.address)).to.equal(
      totalSupply - (amount * fee) / HUNDRED_PERCENT
    );
    expect(await dgvcProxy.balanceOf(feeReceiver.address)).to.equal(
      (amount * partFee) / HUNDRED_PERCENT
    );
  ...
  it("should be possible reach 15 trade cycles with needed checks and burn with rebase WITH admin burns, check latest total supply", async function () {
    ...
    expect(await dgvcProxy.balanceOf(feeReceiver.address)).to.equal(
      BNtoBigInt(feeReceiverBalance) - amountToBurn
    );
    expect(await dgvcProxy.totalSupply()).to.equal(
      BNtoBigInt(supplyBeforeBurn) - amountToBurn
    );
    expect(await dgvcProxy.totalBurn()).to.equal(
      BNtoBigInt(totalBurnBeforeBurn) + amountToBurn
    );
    expect(await dgvcProxy.totalBurn()).to.equal(
      BNtoBigInt(totalBurnWithFeesBeforeBurn) + amountToBurn
    );
    ...
    const rebaseAmount = utils.parseUnits("4000", baseUnit).toBigInt();
    const totalSupplyExpected =
      supplyBeforeRebase + rebaseAmount - (amount * fee) / HUNDRED_PERCENT;
    ...
    const balanceFeeReceiver = BNtoBigInt(
      await dgvcProxy.balanceOf(feeReceiver.address)
    );
    expect(balanceOwner + balanceUser + balanceFeeReceiver).to.equal(
      totalSupplyExpected - 2n
    );
  ...
  it("should be possible to transfer tokens via transferFrom, fees set to 5%, 2.5% - to burn and 2.5% fot, trade cycle updated", async function () {
    ...
    await dgvcProxy
      .connect(userTwo)
      .transferFrom(owner.address, user.address, amount);
  ...
  it("should NOT be possible to transfer tokens via transferFrom, if sender does not have enough allowance", async function () {
    ...
    await expect(
      dgvcProxy
        .connect(userTwo)
        .transferFrom(owner.address, user.address, amount)
    ).to.revertedWith("transfer amount exceeds allowance");
